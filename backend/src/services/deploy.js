...
const UPSERT_PREVIEW = db.kind === 'mysql'
  ? `INSERT INTO preview_deployments (id, project_id, deployment_id, kind, branch, pr_number, subdomain, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)
     ON DUPLICATE KEY UPDATE deployment_id = VALUES(deployment_id), status = 'active', updated_at = VALUES(updated_at)`
  : `INSERT INTO preview_deployments (id, project_id, deployment_id, kind, branch, pr_number, subdomain, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)
...
async function ensureProjectDns(project, subdomain, log) {
  if (project.cf_zone_id && project.cf_tunnel_cname) {
    const owner = await db.prepare('SELECT cf_token FROM users WHERE id = ?').get(project.user_id);
    ...
    if (ownerCfToken) {
      ...
      if (recordId && recordId !== project.cf_record_id) {
        await db.prepare('UPDATE projects SET cf_record_id = ? WHERE id = ?').run(recordId, project.id);
      }
  ...
  const token = await getSecretSetting('default_cf_token');
  const zoneId = await getSetting('default_zone_id');
...
async function setStatus(id, status, extra = {}) {
  ...
  values.push(id);
  await db.prepare(`UPDATE deployments SET ${fields.join(', ')} WHERE id = ?`).run(...values);
}
...
export async function createDeployment(project, { type = 'production', branch, prNumber = null, trigger = 'manual' } = {}) {
  const quotaCheck = await checkDeployQuota(project.user_id);
  ...
  if (type === 'preview') {
    ...
    await db.prepare(UPSERT_PREVIEW).run(pid, project.id, id, kind, b, prNumber, subdomain, ts, ts);
  }

  const deployment = await db.prepare(`SELECT * FROM deployments WHERE id = ?`).get(id);
  enqueue(() => runPipeline(deployment.id), { type: 'deploy', id });
  return deployment;
}

export async function runPipeline(deploymentId) {
  const deployment = await db.prepare(`SELECT * FROM deployments WHERE id = ?`).get(deploymentId);
  if (!deployment) return;
  const project = await db.prepare(`SELECT * FROM projects WHERE id = ?`).get(deployment.project_id);
  ...
  try {
    ...
    const owner = await db.prepare('SELECT github_token FROM users WHERE id = ?').get(project.user_id);
    const cloneToken =
    ...
    await setStatus(deploymentId, 'cloning', { commit_sha: sha, commit_msg: message });
    log(`✓ Checked out ${sha.slice(0, 8)} — ${message}`);

    await importRepoEnv(project.id, dir, log);
    ...
    await setStatus(deploymentId, 'deploying', { image_tag: tag });
    ...
    const env = await envFor(project, deployment.type);
...
async function supersedeContainers(projectId, subdomain, keepDeploymentId) {
  ...
  for (const c of old) {
    ...
    await db.prepare(`UPDATE containers SET status = 'stopped' WHERE id = ?`).run(c.id);
    await db.prepare(`UPDATE deployments SET status = 'stopped' WHERE id = ? AND status = 'running'`).run(c.deployment_id);
    stopRuntimeLogs(c.deployment_id);
    if (config.autoCleanup && c.deployment_id) {
      const dep = await db.prepare(`SELECT image_tag FROM deployments WHERE id = ?`).get(c.deployment_id);
...
export async function stopDeployment(deploymentId) {
  ...
  await db.prepare(`UPDATE containers SET status = 'stopped' WHERE deployment_id = ?`).run(deploymentId);
  await setStatus(deploymentId, 'stopped', { finished_at: now() });
  stopRuntimeLogs(deploymentId);
...
export async function destroyPreview(project, { branch, prNumber }) {
  ...
  for (const c of containers) {
    ...
    await removeContainer(c.name);
    await db.prepare(`UPDATE containers SET status = 'stopped' WHERE id = ?`).run(c.id);
    await db.prepare(`UPDATE deployments SET status = 'stopped' WHERE id = ?`).run(c.deployment_id);
    stopRuntimeLogs(c.deployment_id);
  }
  await db.prepare(
...
export async function resumeOnBoot() {
  const live = await db.prepare(`SELECT * FROM deployments WHERE status = 'running'`).all();
  for (const d of live) {
