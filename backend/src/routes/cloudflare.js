...
async function conn(userId) {
  const row = await db.prepare('SELECT cf_token, cf_account FROM users WHERE id = ?').get(userId);
...
async function resolveTunnelTarget(token, account) {
  const manual = await getSetting('cf_tunnel_cname');
...
export default async function cloudflareRoutes(fastify) {
  ...
  fastify.get('/status', async (request) => {
    const c = await conn(request.user.sub);
    return {
      connected: Boolean(c?.cf_token),
      account: c?.cf_account || null,
      tunnel_cname: await getSetting('cf_tunnel_cname') || null,
  ...
  fastify.post('/connect', async (request, reply) => {
    ...
    try {
      ...
      await db.prepare('UPDATE users SET cf_token = ?, cf_account = ? WHERE id = ?').run(encryptSecret(token), account, request.user.sub);
  ...
  fastify.post('/disconnect', async (request) => {
    await db.prepare('UPDATE users SET cf_token = NULL, cf_account = NULL WHERE id = ?').run(request.user.sub);
    return { ok: true };
  });
  ...
  fastify.post('/tunnel', async (request) => {
    ...
    await setSetting('cf_tunnel_cname', cname);
  ...
  fastify.delete('/project/:id/domain', async (request, reply) => {
    ...
    await db.prepare('UPDATE projects SET cf_zone_id = NULL, cf_zone_name = NULL, cf_record_id = NULL WHERE id = ?').run(p.id);
    return { ok: true };
  });
}
