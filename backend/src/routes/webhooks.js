...
function audit({ projectId, event, action, deliveryId, ref, ok, message }) {
  db.prepare(
...
export default async function webhookRoutes(fastify) {
  fastify.post('/github/:projectId', async (request, reply) => {
    ...
    if (!project) {
      audit({ event, deliveryId, ok: false, message: 'unknown project' });
    ...
    if (!verifyGithubSignature(raw, signature, secret)) {
      audit({ projectId: project.id, event, deliveryId, ok: false, message: 'bad signature' });
    ...
    if (event === 'ping') {
      audit({ projectId: project.id, event, deliveryId, ok: true, message: 'pong' });
    ...
    if (event === 'push') {
      ...
      if (payload.deleted) {
        audit({ projectId: project.id, event, action: 'deleted', deliveryId, ref, ok: true, message: 'branch deleted' });
      ...
      if (branch === project.branch) {
        deployment = createDeployment(project, { type: 'production', branch, trigger: 'webhook' });
      } else if (project.preview_enabled) {
        deployment = createDeployment(project, { type: 'preview', branch, trigger: 'webhook' });
      } else {
        audit({ projectId: project.id, event, action: 'push', deliveryId, ref, ok: true, message: 'previews disabled' });
        return { ok: true, skipped: 'previews disabled' };
      }
      audit({ projectId: project.id, event, action: 'push', deliveryId, ref, ok: true, message: `deploy ${deployment.id}` });
    ...
    if (event === 'pull_request') {
      ...
      if (!project.preview_enabled) {
        audit({ projectId: project.id, event, action, deliveryId, ref, ok: true, message: 'previews disabled' });
        return { ok: true, skipped: 'previews disabled' };
      }

      if (['opened', 'synchronize', 'reopened'].includes(action)) {
        const deployment = createDeployment(project, { type: 'preview', branch, prNumber, trigger: 'webhook' });
        audit({ projectId: project.id, event, action, deliveryId, ref, ok: true, message: `preview ${deployment.id}` });
        return { ok: true, deployment_id: deployment.id, subdomain: deployment.subdomain };
      }
      ...
      if (action === 'closed') {
        if (project.auto_destroy_pr) {
          ...
          audit({ projectId: project.id, event, action, deliveryId, ref, ok: true, message: `destroyed ${res.subdomain}` });
          return { ok: true, destroyed: res };
        }
        audit({ projectId: project.id, event, action, deliveryId, ref, ok: true, message: 'auto-destroy off' });
    ...
    audit({ projectId: project.id, event, deliveryId, ok: true, message: 'ignored event' });
